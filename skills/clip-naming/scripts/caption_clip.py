...
{{
 ...
 "一句话": "15 字以内中文",
 "简短命名": "按用户的命名习惯给这段起名，**6–20 个汉字**，要详细：写清「主体 + 动作 + 对象 + （场景/视角）」，让人一眼知道这段能用在片子哪一拍、后期找素材时好检索。连续动作用 + 连接，视角用（特）（俯）（侧）（跟随）标注，场景（发动机/车底/洗手池/下水道/桌面）要写进去。不要写成句子、不要标点。示例：单灯伸弯开灯照发动机深入吸出细钉子（特） / 指甲油瓶滴胶包裹套筒（特） / 双手展示八支磁吸灯+引下单（俯） / 后吸住伸进缝隙照亮深处钥匙",
